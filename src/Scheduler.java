...
public class Scheduler {
    ...
    private void scheduleAppointment(String[] sInput) {
        ...
        if(dob.isAfterToday()){
            System.out.println("Patient dob: " +dobInput+" is today or a date after today.");
            return;
        }
        ...
        if (isProviderFreeForTimeslot(provider, timeslot, appointmentDate, patientProfile)==2) {
            System.out.println(provider.toString() + " is not available at slot "+ timeslotInput+ ".");
            return;
        }
        if (isProviderFreeForTimeslot(provider, timeslot, appointmentDate, patientProfile)==3) {
    ...
    private int isProviderFreeForTimeslot(Provider provider, Timeslot timeslot, Date appointmentDate, Profile patient) {
        for (int i = 0; i < appointmentList.size(); i++) {
            ...
            if (appointment.getProvider().equals(provider) && appointment.getTimeslot().equals(timeslot)
                ...
                if (appointment.getPatient().equals(patient)) {
                    return 3; //3= patient has an existing appointment at the same time slot
                }
                return 2; //2= provider is not available at the slot
            }
        }
        return 1; //1= provider is available at the slot
    }
    ...
    public void rescheduleAppointment(String[] sInput) {
        ...
        if (isProviderFreeForTimeslot(provider, newTimeslot, appointmentDate, patientProfile) == 2) {
...
private Appointment findAppointment(Date date, Timeslot timeslot, Profile patientProfile) {
    ...
    private void printBillingStatements() {
        ...
        for (Appointment allAppointment : allAppointments) {
            if(allAppointment == null) break;
            Profile patientProfile = allAppointment.getPatient();
            Specialty specialty = allAppointment.getProvider().getSpecialty();
